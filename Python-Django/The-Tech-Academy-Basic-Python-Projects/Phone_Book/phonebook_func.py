...
def center_window(self, w, h): # pass tkinter frame (master) reference and the w and h
    # user's window
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    # calculate x and y coodinate to paint the app centered on the user's screen
    x = int((screen_width/2) - (w/2))
    y = int((screen_height/2) - (h/2))
    centerGeo = self.master.geometry('{}x{}+{}+{}'.format(w, h, x, y))
    return centerGeo

# catch if the user's click on the window upper-right 'X' to ensure they want to close
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit applcation?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
def create_db(self):
    ...
    with conn:
        ...
        # You must commit() to save changes & close to the database connection
        conn.commit()
    conn.close()
    first_run(self)
...
def onSelect(self,event):
    #calling the eve is the self.lstList1 widget
...
def addToList(self):
    ...
    # normalize the adata to keep it consistent in the database
    var_fname = var_fname.strip() # This will remove any blank spaces befoer and after the user's entry
    var_lname = var_lname.strip() # This will ensure that the first character in each word is capitalized
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normaliezed names into a fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            cursor = conn.cursor()
            # check the db for esistance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is  then there is no existance of the fullname and we can add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        # the database...cannot delete last recor or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
#####               onRefresh(self) # update the listbox fo the changes
                conn.commit()
        else:
...
def onUpdate(self):
    try:
        var_select = self.lstList1.curselect()[0] # index of the list selection
        var_value = self.lstList1.get(var_select) # list selection's text value
    except:
        messagebox.showinfo("Missing selection","Noname was selected from the list box. \nCancelling the Update request.")
        return
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            # the database...meaning, there are not changes to update.
