...
from BoardGame.models import Game
from BoardGame.forms import GameForm, SearchForm
...
def addHot(request, id):
    ...
    id = int(id)
    query = {"id": id}
    # Make a request to the BoardGameeek API for information regarding a specific game
    ...
    if not Game.objects.filter(game_name=game.find('name').get('value')).exists():
        ...
        game_designer = game_publisher = game_playtime = "" #Placeholder value if data is not present
        game_min_players = game_max_players = 0             #Placeholder value if data is not present
        ...
        for link in game.iter('link'):
            # Sets the publisher value if it has not already been set (some games have publisher listed in multiple countries)
