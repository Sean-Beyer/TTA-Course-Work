...
// this function will roll for rand number twice, one for
// eache player and determine which player won the roll
function rollForTurn() {
    ...
    for (i=0;i<xArray.length;i++) {
        ...
        setTimeout (function() {writeMsg(txt1);}, 1000); //timed delay for dramatic effect
    }
...
//--------------------------------------------------------------
// initiat the game, roll for turn & determine the active player
//--------------------------------------------------------------
function startGame() {
    var xTurn = 0;
    ...
    // assign proper state of the conrol buttons
    ...
    // assign the Active player to the con
...
function btnDisabled(btn) {
    btn.style.color = "fff";
    btn.style.boarder = "2ps solid rgb(153, 153,102)";
...
function stopEnabled(btn) {
    btn.style.color = "#fff";
    btn.style.boarder - "2px solid rgb(204, 0, 0)";
...
function startEnabled(btn) {
    btn.style.color = "#fff";
    btn.style.boarder = "2px solid rgb(0, 153, 0)";
    btn.style.backgroundColor = "rgb(57, 250, 0)";
    btn.disabled = false;
}
...
// this function will cancel the message console from view
function hideGameMsg() {
    clearMsg() //clear the text from the essage console
...
// proposed avatar assignments and prevent them form being the same.
...
// thise function will return the active player's avatar
function determineAvatar() {
    // determine the correct avatar to point for the active player
...
// and check the proposed move for a volidity
...
// as squares are selected they check in with this funktion to see if that particular
...
function recordMoves(square) {
    ...
    var info = boardState.split(','); // separate the string by commas to creat an array
...
// and the concentrate the current move to it.
...
function checkForWinCon() {
    ...
    for (var i in info) {
        squareArray.push(info[i].charAt(0)); // new array with only square not avatars
    }
    // call this following array of function to check for anyof the possible win cons
...
function check4Tie() {
    ...
    boardState = boardState.substring(1); // remove leasing comma
...
// these functions will produece game sounds depending on the occasion
function squareSound() {
    var sound = document.getElementById("placeAvatar");
    sound.play();
    setTimeout(function() {sound.pause();}, 400); // add delay to these to keep
...
// call this function to make entire backgourn color
// flash for a few second for a win animation
function blink() {
...
function checkWinCon1(info,squareArray) {
    ...
    // gametime searching for the existance of
...
function checkWinCon2(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "3") {
            var match3Avatar = info[i].charAt(1); // only interested in recordning the avatar
        }
...
function checkWinCon3(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "6") {
            var match6Avatar = info[i].charAt(i); // only interested in recording the avatar
        }
...
// checking for wincon square 258
...
// checking for win con squares 642
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied.
            ...
            // build new array adding the newly selsected square and the assigned avatar
...
function square2Animate() {
    var activePlayer = document.getElementById('showPlayer').innerHTML;
    if (activePlayer != "Game Stopped") { // if game has not started prevent avatar placement
        var square = "1"; // identify the square selected
        // check if the proposed square is valid
        var verdict = recordMoves(square);
        if (verdict == undefined) { //if verdict is empty than the square is uoccupied.
            var paintAvatar = determineAvatar(); // get the correct avatar to paint for the active player
            var selected = document.getElementsByClassName(paintAvatar)[1];
            if (paintAvatar == "O") { // change these all to ternary statements instead
                animateO(selected); // call function to animate O
            } else if (paintAvatar == "X") {
                animateX(selected); // call function animate X
            }
...
// this function with perform the animation for the O avatar.
